create or replace FUNCTION         FN_GET_NODE_OLT_NAME
(
...
BEGIN
    ...
    if NASPORTID is null then
        ...
        if substr(t_hf_port,0,4) = 'xpon'  then
            select SUBSTR(NASPORTID,1,INSTR(NASPORTID, ' ') - 1) into t_port_group from dual;
    ...
    --t_port_group := 'FTTX-AIS'; -- จ.อำนาจเจริญ    
